...
def dismiss_disclaimer(page):
    selectors = ["button:has-text('AGREE')", "button:has-text('Agree')", "button:has-text('OK')"]
...
def select_date(page, date_string):
    ...
    for _ in range(24):
        ...
        if y_val == year and month_map.get(m_name) == month: break
...
def main():
    ...
    with sync_playwright() as p:
        try:
            ...
            if select_station(page, "#dest_from", FROM_STATION) and \
               select_station(page, "#dest_to", TO_STATION) and \
               select_date(page, JOURNEY_DATE_INPUT):
